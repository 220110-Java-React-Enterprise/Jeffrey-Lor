...
public class Subsequence {
    ...
    public static int subsequence(List<Integer> seq, List<Integer> nums) {
        ...
        if((nums.isEmpty())) {
            return seq.size();
        // If the next number is less than our last inserted number, return what we've got
        } else if(!seq.isEmpty() && nums.get(0) <= seq.get(seq.size() - 1)) {
            return subsequence(seq, nums.subList(1, nums.size()));
        }
